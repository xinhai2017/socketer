...
def get_default_params():
    return tf.contrib.training.HParams(
        ...
        d_channels = [32, 64, 128, 256], # 卷积核通道数
        batch_size = 128,
        learning_rate = 0.002,
        beta1 = 0.5, # adamoptimization 参数
        img_size = 32,   # 生成目标图片尺寸
    )
...
class MnistData(object):
    ...
    def next_batch(self, batch_size):
        ...
        if end_indicator > self._example_num:
            self._random_shuffle()
            self._indicator = 0
            self._indicator = self._indicator + batch_size
    ...
    def __call__(self, inputs, training):  # 魔法方法，类的对象可以当作函数使用
        ...
        with tf.variable_scope('generator', reuse=self._reuse):
            ...
            with tf.variable_scope('inputs_conv'):
                ...
                conv0 = tf.reshape(fc,
                                   [-1,  # batch_size
                                    ...
                                    self._init_conv_size,
                                    self._channels[0]])
                bn0 = tf.layers.batch_normalization(conv0, trainable=training)
            ...
            for i in range(1, len(self._channels)):  # 做反卷积
                ...
                deconv_inputs = conv2d_transpose(deconv_input,
                                                 self._channels[i],
                                                 "deconv-%d" % i,
                                                 training,
                                                 with_bn_relu)
        ...
        self._reuse = True
        self.variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,  # 保存生成器的所有参数
                                           scope='generator')
        return imgs
