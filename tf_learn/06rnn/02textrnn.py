...
def get_default_params():
    return tf.contrib.training.HParams( # 返回一个对象
               num_embedding_size = 16,  #  32 每个词语对应的向量长度
               ...
               batch_size = 100,
               clip_lstm_grads = 1.0,    # 防止梯度爆炸，控制lstm的梯度
               learning_rate = 0.001,
...
class Vocab:
    ...
    @property #之后调用可以像调成员变量一样调用，而非像调函数一样调用
    def unk(self):
        return self._unk
...
def create_model(hps, vocab_size, num_classes):
    ...
    # embedding层
    embedding_initializer = tf.random_uniform_initializer(-1.0, 1.0)  # 初始化函数，均匀分布-1到1之间
    ...
    lstm_init = tf.random_uniform_initializer(-scale, scale) #初始化函数，均匀分布
